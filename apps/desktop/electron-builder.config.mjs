...
export function createElectronBuilderConfig(
  ...
  // Local Harness-CN builds ship without an EV certificate or an updater origin, so the
  // signer, the update channel, and the publish block are all omitted together.
  const unsigned = env.DSH_DESKTOP_UNSIGNED === '1'
  if (unsigned && resolvedPlatform !== 'win32') {
    throw new Error('desktop package: unsigned builds require Windows')
  }
  ...
  return {
    appId,
    productName: 'Harness-CN',
    artifactName: 'harness-cn-${version}-${os}-${arch}.${ext}',
    directories: { output: buildPaths.artifacts, buildResources: 'build' },
    asar: true,
    // Chromium otherwise ships all 55 locale packs (~48 MB). The shell copy and the dsh
    // client resolve to English or Chinese, so the remaining packs are dead weight.
